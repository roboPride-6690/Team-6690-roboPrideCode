...
public class TeleopDrive extends CommandBase {
  ...
  public void execute() {
    ...
    if(vY.getAsDouble() > 0.5){
      yVelocity = Math.pow(vY.getAsDouble(), 3) * -Drivebase.MAX_SPEED;
    } 
    else{
      yVelocity = Math.pow(vY.getAsDouble(), 3) * -Drivebase.MAX_SPEED/2;
    } 

    if(vX.getAsDouble() > 0.5){
      xVelocity = Math.pow(vX.getAsDouble(), 3) * Drivebase.MAX_SPEED;
    } 
    else{
      xVelocity = Math.pow(vX.getAsDouble(), 3) * Drivebase.MAX_SPEED/2;
    }
